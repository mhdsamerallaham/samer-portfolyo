...
const fallbackFAQDrafts = [
  {
    ...
    short_answer_tr:
      "Sepeti terk etme oranını düşürmek için şeffaf kargo ücretleri sunmalı, misafir ödeme (guest checkout) seçeneği eklemeli ve tek sayfada ödeme (one-page checkout) düzenine geçmelisiniz.",
    content_tr:
    ...
    who_is_this_for_tr:
      "Shopify mağazasında yüksek sepet terki yaşayan e-ticaret yöneticileri ve marka sahipleri için.",
    ...
    short_answer_en:
      "To reduce cart abandonment, offer transparent shipping costs, enable guest checkout, and switch to a one-page checkout layout.",
    content_en:
      "<h3>Cart Abandonment Strategies</h3><p>Over 70% of e-commerce shoppers abandon their carts before purchase due to hidden fees or complex checkout steps.</p>",
    ...
    short_answer_ar:
      "لتقليل نسبة ترك السلة، قدم تكاليف شحن شفافة، وفعل خيار الدفع كزائر، واستخدم صفحة دفع واحدة.",
    content_ar:
      "<h3>استراتيجيات تقليل ترك السلة</h3><p>أكثر من 70٪ من متسوقي التجارة الإلكترونية يتركون سلاتهم قبل الشراء بسبب الرسوم المخفية أو خطوات الدفع المعقدة.</p>",
  ...
  },
  {
    ...
    short_answer_tr:
      "Next.js e-ticaret sitenizi hızlandırmak için Sunucu Tarafı İşleme (SSR) veya Statik Üretim (SSG) kullanmalı, görselleri next/image bileşeni ile optimize etmeli ve gereksiz JavaScript paketlerini elemelisiniz.",
    content_tr:
      "<h3>Next.js E-Ticaret Hız Optimizasyonu</h3><p>Google Core Web Vitals metriklerini yeşile çevirmek için dinamik sayfalarınızda ISR (Incremental Static Regeneration) mimarisini tercih edebilirsiniz.</p>",
    who_is_this_for_tr:
      "Yüksek hızlı ve ölçeklenebilir e-ticaret altyapısı kurmak isteyen frontend ve fullstack yazılımcılar için.",
    ...
    short_answer_en:
      "To speed up Next.js e-commerce sites, leverage SSR or SSG, optimize images with next/image, and eliminate heavy JS bundles.",
    content_en:
      "<h3>Next.js Speed Optimization</h3><p>Utilize Incremental Static Regeneration (ISR) to achieve sub-second load times and 95+ Core Web Vitals scores.</p>",
    ...
    short_answer_ar:
      "لتسريع موقع Next.js، استخدم العرض على جانب الخادم SSR أو التوليد الثابت SSG، وقم بتحسين الصور كودياً.",
    content_ar:
      "<h3>تحسين سرعة Next.js</h3><p>استخدم ISR لتحقيق سرعة تحميل فائقة في أقل من ثانية واحدة.</p>",
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      console.warn(
        "[FAQ Generator] All LLM providers timed out or failed. Loading fail-safe fallback draft...",
        llmErr.message
      );
